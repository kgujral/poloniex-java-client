...
public class PoloniexPublicAPIClient implements PriceDataAPIClient {
  ...
  public String returnTicker() {
    try {
      ...
      LogManager.getLogger(PoloniexPublicAPIClient.class)
          .warn("Call to return ticker API resulted in exception - " + ex.getMessage(), ex);
    }
  ...
  private String getChartData(String currencyPair, String startEpochInSec, String endEpochInSec, String periodInSec) {
    try {
      String url = PUBLIC_URL + "command=returnChartData&currencyPair=" + currencyPair + "&start=" + startEpochInSec
          + "&end=" + endEpochInSec + "&period=" + periodInSec;
      return client.getHttp(url, null);
    } catch (IOException ex) {
      LogManager.getLogger(PoloniexPublicAPIClient.class)
          .warn("Call to Chart Data API resulted in exception - " + ex.getMessage(), ex);
    }
