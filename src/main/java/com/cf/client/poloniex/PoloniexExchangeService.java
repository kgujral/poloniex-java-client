...
public class PoloniexExchangeService implements ExchangeService {
   ...
   *          The candlestick chart data period. Valid values are 300 (5 min), 900
   *          (15 minutes), 7200 (2 hours), 14400 (4 hours), 86400 (daily)
   * @param startEpochInSeconds
   *          UNIX timestamp format and used to specify the start date of the data
   *          returned
  ...
  /**
   * * Returns the complete balances inclusive non-zero balances or not depending
   * on parameter includeZeroBalances
   * 
   * @param includeZeroBalances
   *          The includeZeroBalances
   ...
   * @param amount
   * @param fillOrKill
   *          Will either fill in its entirety or be completely aborted
   * @param immediateOrCancel
   *          Order can be partially or completely filled, but any portion of the
   *          order that cannot be filled immediately will be canceled rather than
  ...
  public PoloniexOrderResult sell(String currencyPair, BigDecimal sellPrice, BigDecimal amount, boolean fillOrKill,
    ...
    try {
      String sellTradeResult = tradingClient.sell(currencyPair, sellPrice, amount, fillOrKill, immediateOrCancel,
          postOnly);
  ...
  public PoloniexOrderResult buy(String currencyPair, BigDecimal buyPrice, BigDecimal amount, boolean fillOrKill,
    ...
    PoloniexOrderResult orderResult = null;
    try {
      String buyTradeResult = tradingClient.buy(currencyPair, buyPrice, amount, fillOrKill, immediateOrCancel,
          postOnly);
  ...
  @Override
  public PoloniexOrderResult moveOrder(String orderNumber, BigDecimal rate, Boolean immediateOrCancel,
      Boolean postOnly) {
    long start = System.currentTimeMillis();
    PoloniexOrderResult orderResult = null;
    try {
